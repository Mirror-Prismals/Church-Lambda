...
// -----------------------------------
// 4) Tromp-Style Corner Diagrams with Color Coding
// -----------------------------------

// Extended line segment with color (r, g, b)
struct LineSegment {
    float x1, y1, x2, y2;
    float r, g, b;
};
...
// Build variable diagram (colored red)
Diagram buildCornerVar(const std::string& v) {
    Diagram d;
    d.width = BASE_VAR_SIZE;
    d.height = BASE_VAR_SIZE;
    // Red: (1, 0, 0)
    d.lines.push_back({ 0, 0, BASE_VAR_SIZE, 0, 1.f, 0.f, 0.f });
    d.lines.push_back({ 0, 0, 0, -BASE_VAR_SIZE, 1.f, 0.f, 0.f });
    return d;
}

// Build lambda diagram (outer box colored green)
Diagram buildCornerLam(const std::string& var, const ExprPtr& body) {
    ...
    Diagram d;
    // Outer box in green: (0, 1, 0)
    d.lines.push_back({ 0, 0, w, 0, 0.f, 1.f, 0.f });
    d.lines.push_back({ w, 0, w, -h, 0.f, 1.f, 0.f });
    d.lines.push_back({ 0, -h, w, -h, 0.f, 1.f, 0.f });
    d.lines.push_back({ 0, 0, 0, -h, 0.f, 1.f, 0.f });
...
// Build application diagram (outer box colored blue)
Diagram buildCornerApp(const ExprPtr& f, const ExprPtr& a) {
    ...
    Diagram d;
    // Outer box in blue: (0, 0, 1)
    d.lines.push_back({ 0, 0, w, 0, 0.f, 0.f, 1.f });
    d.lines.push_back({ w, 0, w, -h, 0.f, 0.f, 1.f });
    d.lines.push_back({ 0, -h, w, -h, 0.f, 0.f, 1.f });
    d.lines.push_back({ 0, 0, 0, -h, 0.f, 0.f, 1.f });
    mergeDiagram(d, df);
    mergeDiagram(d, da);
    d.width = w;
    d.height = h;
...
// Interpolate vertices arrays (each vertex now has 5 floats: x, y, r, g, b)
...
int main() {
    ...
    // Each vertex now carries 5 floats: x, y, r, g, b
    ...
    for (size_t i = 0; i < steps.size(); i++) {
        ...
        // For each line, push two vertices (each with 5 floats)
        for (auto& ln : d.lines) {
            diagVerts[i].push_back(ln.x1);
            diagVerts[i].push_back(ln.y1);
            diagVerts[i].push_back(ln.r);
            diagVerts[i].push_back(ln.g);
            diagVerts[i].push_back(ln.b);
        ...
        if (d.lines.empty()) {
            // Fallback to a single point (with white color)
            diagVerts[i].push_back(0.f);
            diagVerts[i].push_back(0.f);
            diagVerts[i].push_back(1.f);
            diagVerts[i].push_back(1.f);
            diagVerts[i].push_back(1.f);
            diagVerts[i].push_back(0.f);
            diagVerts[i].push_back(0.f);
    ...
    // Set up vertex attributes: location 0: position (2 floats), location 1: color (3 floats)
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        glUseProgram(shader);
        glBindVertexArray(VAO);
        int vertexCount = (int)diagVerts.at(currentIndex).size() / 5;
        // If animating, use interpolated vertex count (should be same as others)
        if (animating) {
            vertexCount = (int)(std::max(diagVerts.at(currentIndex).size(), diagVerts.at(targetIndex).size()) / 5);
        }
